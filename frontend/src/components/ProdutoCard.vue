...
<template>
  <div class="card-container">
    <div class="overflow-hidden rounded-t-2xl relative cursor-pointer group"
      :style="{ background: 'var(--bg)' }"
      @click="irParaProduto">

      <div class="aspect-square overflow-hidden">
        <img v-if="produto.imagem_url"
      ...
      <button @click.stop="emit('favoritar')"
        class="absolute top-2.5 right-2.5 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 glass"
    ...
    <div class="p-3 sm:p-4 cursor-pointer" :style="{ background: 'var(--bg-card-solid)' }"
      @click="irParaProduto">
      <p class="font-display font-semibold text-sm sm:text-base truncate" :style="{ color: 'var(--text-bright)' }">
        {{ produto.nome }}
      </p>
      <p v-if="produto.descricao" class="text-xs mt-0.5 line-clamp-2 leading-relaxed"
      ...
      <div class="flex items-center justify-between mt-2.5 pt-2.5"
        ...
        <span v-if="estoqueBaixo" class="text-[10px] font-medium"
          :style="{ color: 'var(--ctp-yellow)' }">Apenas {{ produto.estoque }}</span>
      </div>
    </div>
  </div>
</template>
...
.card-container:hover {
  ...
  box-shadow: 0 12px 40px var(--shadow), 0 4px 12px var(--shadow);
}
