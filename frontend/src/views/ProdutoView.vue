...
<template>
  <div class="min-h-screen" :style="{ background: 'var(--bg)' }">
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 py-8 pb-20">
      ...
      <div v-else-if="erro" class="flex flex-col items-center justify-center py-32 text-center animate-fade-up">
      ...
      <div v-else-if="produto" class="animate-fade-up">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          <div class="glass-strong rounded-2xl overflow-hidden">
            <div class="relative aspect-square" :style="{ background: 'var(--bg)' }">
              <img v-if="imagens.length > 0"
                ...
                :alt="produto.nome"
                class="w-full h-full object-contain p-6 sm:p-8 transition-all duration-300"
                :style="{ background: 'var(--bg-card-solid)' }" />
              <div v-else class="w-full h-full flex items-center justify-center"
                :style="{ background: 'var(--accent-soft)' }">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                  :style="{ color: 'var(--accent-soft)' }" stroke-width="1">
          ...
          <div class="glass-strong rounded-2xl p-6 sm:p-8 animate-fade-up"
            :style="{ animationDelay: '0.2s' }">

            <div class="flex items-start justify-between gap-4 mb-3">
              <span class="text-[11px] font-semibold uppercase tracking-[0.2em]"
                :style="{ color: 'var(--accent)' }">{{ produto.categoria }}</span>
              <span v-if="!esgotado" class="text-xs font-semibold px-3 py-1 rounded-full flex-shrink-0"
                :style="{ background: 'var(--accent-soft)', color: 'var(--accent)' }">{{ produto.estoque }} disponíveis</span>
              <span v-else class="text-xs font-semibold px-3 py-1 rounded-full flex-shrink-0"
                :style="{ background: 'rgba(var(--ctp-red), 0.1)', color: 'var(--ctp-red)' }">Esgotado</span>
            </div>

            <h1 class="font-display text-2xl sm:text-3xl font-semibold leading-snug mb-4"
              :style="{ color: 'var(--text-bright)' }">
              {{ produto.nome }}
            </h1>

            <p v-if="produto.descricao" class="text-sm leading-relaxed mb-6"
              :style="{ color: 'var(--text-dim)' }">
              {{ produto.descricao }}
            </p>
            ...
            <div class="border-t pt-6" :style="{ borderColor: 'var(--border)' }">
              ...
              <a v-if="produto.purchasePrice !== null && produto.purchasePrice !== undefined"
                class="block text-xs mt-1" :style="{ color: 'var(--text-dim)' }">
                Custo: R$ {{ produto.purchasePrice.toFixed(2) }}
                <span v-if="produto.profitMargin !== null && produto.profitMargin !== undefined">
            ...
            <div class="mt-6 space-y-3">
              <a :href="esgotado ? undefined : WHATSAPP_LINK" target="_blank" rel="noopener noreferrer"
                ...
                class="flex items-center justify-center gap-3 w-full py-4 rounded-xl font-semibold text-base transition-all duration-200"
                :style="esgotado
