...
def _validar_imagem(imagem: UploadFile, conteudo: bytes) -> None:
    if imagem.content_type not in TIPOS_PERMITIDOS:
        raise HTTPException(status_code=400, detail=f"Tipo não permitido. Use: jpeg, png ou webp")
...
async def criar_produto(
    ...
    if imagens:
        # Limitar a 4 imagens
        imagens_validas = [img for img in imagens if img.filename][:MAX_IMAGENS]
        for ordem, imagem in enumerate(imagens_validas):
            conteudo = await imagem.read()
            if not conteudo:
                continue
            _validar_imagem(imagem, conteudo)
            nome_arquivo = f"produto_{produto.id}_{uuid.uuid4().hex[:8]}"
...
async def adicionar_imagens(
    ...
    imagens_atuais = len(produto.imagens)
    espacos = MAX_IMAGENS - imagens_atuais
    if espacos <= 0:
        raise HTTPException(status_code=400, detail=f"Produto já tem {MAX_IMAGENS} imagens")

    imagens_validas = [img for img in imagens if img.filename][:espacos]
    for i, imagem in enumerate(imagens_validas):
        conteudo = await imagem.read()
        if not conteudo:
            continue
        _validar_imagem(imagem, conteudo)
